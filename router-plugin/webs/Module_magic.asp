...
<div style="margin:15px 0 5px 0;" class="formfontdesc">7×24保护：用户组网日志上限约 128KB，内部诊断日志上限约 64KB；默认 RSS 超过 64MB，最长约 20 秒内只自动重启 MagicTier 核心程序，绝不会重启路由器；10分钟内连续异常超过3次才停用插件自动运行；进程以较低 CPU 优先级运行。</div>
