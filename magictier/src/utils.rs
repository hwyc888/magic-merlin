...
pub fn init_logger(
    ...
    if file_level != LevelFilter::OFF || need_reload {
        ...
        let file = file_config.file.as_deref().unwrap_or("magictier.log");
...
pub fn setup_panic_handler() {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        let tmp_path = std::env::temp_dir().join("magictier-panic.log");
        let candidate_path = [
            std::path::PathBuf::from_str("magictier-panic.log").ok(),
            Some(tmp_path),
        ];
        let mut file = None;
        ...
        write_err("panic occurred, if this is a bug, please report this issue on github (https://github.com/MagicTier/MagicTier/issues)".to_string());
        write_err(format!("magictier version: {}", crate::VERSION));
