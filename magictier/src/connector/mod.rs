...
pub async fn create_connector_by_url(
    ...
    let mut connector: Box<dyn TunnelConnector + 'static> = match url.scheme() {
        ...
        // 新增的规避技术协议
        "reality" => {
            use crate::tunnel::reality::{RealityConfig, RealityTunnelConnector};
            let dst_addr =
                check_scheme_and_get_socket_addr::<SocketAddr>(&url, "reality", ip_version).await?;
            let config = RealityConfig::from_url(&url);
            let mut connector = RealityTunnelConnector::new(url, config);
            if global_ctx.config.get_flags().bind_device {
                set_bind_addr_for_peer_connector(
                    &mut connector,
                    dst_addr.is_ipv4(),
        ...
        "shadowtls" => {
            use crate::tunnel::shadowtls::{ShadowTlsConfig, ShadowTlsTunnelConnector};
            let dst_addr =
                check_scheme_and_get_socket_addr::<SocketAddr>(&url, "shadowtls", ip_version).await?;
            let config = ShadowTlsConfig::from_url(&url);
            let mut connector = ShadowTlsTunnelConnector::new(url, config);
            if global_ctx.config.get_flags().bind_device {
                set_bind_addr_for_peer_connector(
                    &mut connector,
                    dst_addr.is_ipv4(),
        ...
        #[cfg(feature = "quic")]
        "hysteria2" => {
            use crate::tunnel::hysteria2::{Hysteria2Config, Hysteria2TunnelConnector};
            let dst_addr =
                check_scheme_and_get_socket_addr::<SocketAddr>(&url, "hysteria2", ip_version).await?;
            let config = Hysteria2Config::from_url(&url);
            let mut connector = Hysteria2TunnelConnector::new(url, config);
            if global_ctx.config.get_flags().bind_device {
                set_bind_addr_for_peer_connector(
                    &mut connector,
                    dst_addr.is_ipv4(),
