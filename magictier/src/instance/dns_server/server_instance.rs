// single-instance server in one machine, every magictier instance that has ip address and tun device will try to create a server instance.
...
// all the clients will exit and let the magictier instance to launch a new server instance.
