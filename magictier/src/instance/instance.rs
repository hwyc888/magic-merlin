...
use crate::tunnel::obfuscation::{init_global_obfuscation, ObfuscationConfig};
...
impl Instance {
    pub fn new(config: impl ConfigLoader + 'static) -> Self {
        ...
        if flags.enable_obfuscation {
            let network_identity = global_ctx.config.get_network_identity();
            let obfuscation_config = if flags.obfuscation_magic != 0 {
                // 使用用户指定的魔数
                let mut config = ObfuscationConfig::from_network_secret(
                    network_identity.network_secret_digest.as_ref()
                        .map(|d| d.as_slice())
                        .unwrap_or(&[])
                );
                config.handshake_magic = flags.obfuscation_magic;
                config
            } else {
                // 从网络密钥派生混淆配置
                ObfuscationConfig::from_network_secret(
                    network_identity.network_secret_digest.as_ref()
                        .map(|d| d.as_slice())
                        .unwrap_or(&[])
                )
            };
            init_global_obfuscation(obfuscation_config);
            tracing::info!("[INIT] protocol obfuscation enabled");
        } else {
            // 禁用混淆，使用原始特征码
            init_global_obfuscation(ObfuscationConfig::disabled());
            tracing::info!("[INIT] protocol obfuscation disabled (compatibility mode)");
        }
