...
use crate::{
    ...
    tunnel::{
        ...
        udp::UdpTunnelListener, Tunnel, TunnelError, TunnelListener,
    },
};
...
pub fn get_listener_by_url(
    ...
    Ok(match l.scheme() {
        ...
        // 新增的规避技术协议
        "reality" => {
            use crate::tunnel::reality::{RealityConfig, RealityTunnelListener};
            let config = RealityConfig::from_url(l);
            Box::new(RealityTunnelListener::new(l.clone(), config))
        }
        "shadowtls" => {
            use crate::tunnel::shadowtls::{ShadowTlsConfig, ShadowTlsTunnelListener};
            let config = ShadowTlsConfig::from_url(l);
            Box::new(ShadowTlsTunnelListener::new(l.clone(), config))
        }
        _ => {
...
impl<H: TunnelHandlerForListener + Send + Sync + 'static + Debug> ListenerManager<H> {
    fn listen_retry_delay_secs(err_count: u32) -> u64 {
        let shift = err_count.min(5);
        std::cmp::min(30, 1u64 << shift)
    }

    fn accept_retry_same_listener(err: &TunnelError) -> bool {
        match err {
            TunnelError::IOError(io_err) => {
                if matches!(io_err.raw_os_error(), Some(23) | Some(24)) {
                    // ENFILE/EMFILE: keep listener alive and wait for fd pressure to drop.
                    return true;
                }
    ...
    async fn run_listener(
        ...
        loop {
            ...
            match l.listen().await {
                Ok(_) => {
                    err_count = 0;
                    let local_url = l.local_url();
                    global_ctx.add_running_listener(local_url.clone());
                    global_ctx.issue_event(GlobalCtxEvent::ListenerAdded(local_url.clone()));
                }
                Err(e) => {
                    tracing::error!(?e, ?l, "listener listen error");
                    err_count = err_count.saturating_add(1);
                    let retry_after_secs = Self::listen_retry_delay_secs(err_count);
                    global_ctx.issue_event(GlobalCtxEvent::ListenerAddFailed(
                        l.local_url(),
                        format!(
                            "error: {:?}, retry listen later ({}s, streak={})...",
                            e, retry_after_secs, err_count
                        ),
                    ));
            ...
            loop {
                let ret = match l.accept().await {
                    Ok(ret) => ret,
                    Err(e) => {
                        let retry_same_listener = Self::accept_retry_same_listener(&e);
                        let retry_after_secs = if retry_same_listener { 5 } else { 1 };
                        global_ctx.issue_event(GlobalCtxEvent::ListenerAcceptFailed(
                            l.local_url(),
                            format!(
                                "error: {:?}, retry listen later ({}s, recreate_listener={})...",
                                e, retry_after_secs, !retry_same_listener
                            ),
                        ));
                        tracing::error!(?e, ?l, "listener accept error");
                        tokio::time::sleep(std::time::Duration::from_secs(retry_after_secs))
                            .await;
                        if retry_same_listener {
                            continue;
                        } else {
                            global_ctx.remove_running_listener(&l.local_url());
                            break;
                        }
                    }
                };
