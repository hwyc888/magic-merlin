...
use crate::tunnel::obfuscation::get_global_obfuscation;
...
pub fn u32_to_tid(tid: u32) -> TransactionId {
    ...
    // 使用混淆配置中的 STUN TID 前缀，而不是硬编码的 0xdeadbeef
    let prefix = get_global_obfuscation().stun_tid_prefix;
    tid_arr[..4].copy_from_slice(&prefix.to_be_bytes());
    tid_arr[8..12].copy_from_slice(&tid_buf);
    TransactionId::new(tid_arr)
}

/// 获取当前配置的 STUN TID 前缀字节
pub fn get_stun_tid_prefix_bytes() -> [u8; 4] {
    get_global_obfuscation().stun_tid_prefix.to_be_bytes()
}
