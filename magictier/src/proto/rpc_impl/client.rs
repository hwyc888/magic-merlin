...
impl Client {
    ...
    pub fn run(&self) {
        ...
        // Clean up stale inflight requests that may have been orphaned due to timeouts or errors
        let inflight_requests_cleanup = self.inflight_requests.clone();
        tasks.spawn(async move {
            loop {
                tokio::time::sleep(std::time::Duration::from_secs(30)).await;
                let now = std::time::Instant::now();
                let before_len = inflight_requests_cleanup.len();
                inflight_requests_cleanup.retain(|_, v| {
                    // Remove requests that have been pending for more than 5 minutes
                    now.duration_since(v.start_time) < std::time::Duration::from_secs(300)
                });
                let after_len = inflight_requests_cleanup.len();
                if before_len != after_len {
                    tracing::warn!(
                        "Cleaned up {} stale inflight requests",
                        before_len - after_len
                    );
                }
                inflight_requests_cleanup.shrink_to_fit();
            }
        });
