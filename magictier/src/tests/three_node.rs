...
pub fn get_inst_config(
    ...
    config.set_listeners(vec![
        "tcp://0.0.0.0:52710".parse().unwrap(),
        "udp://0.0.0.0:52710".parse().unwrap(),
        "wg://0.0.0.0:52711".parse().unwrap(),
        "ws://0.0.0.0:52714".parse().unwrap(),
        "wss://0.0.0.0:52715".parse().unwrap(),
    ]);
    ...
    if proto == "tcp" {
        inst1
            ...
            .add_connector(TcpTunnelConnector::new(
                "tcp://10.1.1.2:52710".parse().unwrap(),
            ));
    } else if proto == "udp" {
        inst1
            .get_conn_manager()
            .add_connector(UdpTunnelConnector::new(
                "udp://10.1.1.2:52710".parse().unwrap(),
            ));
    } else if proto == "wg" {
        #[cfg(feature = "wireguard")]
        inst1
            ...
            .add_connector(WgTunnelConnector::new(
                "wg://10.1.1.2:52711".parse().unwrap(),
        ...
        inst1
            ...
            .add_connector(crate::tunnel::websocket::WSTunnelConnector::new(
                "ws://10.1.1.2:52714".parse().unwrap(),
            ));
    } else if proto == "wss" {
        #[cfg(feature = "websocket")]
        inst1
            .get_conn_manager()
            .add_connector(crate::tunnel::websocket::WSTunnelConnector::new(
                "wss://10.1.1.2:52715".parse().unwrap(),
            ));
    }
...
pub async fn proxy_three_node_disconnect_test(#[values("tcp", "wg")] proto: &str) {
    ...
    if proto == "tcp" {
        inst4
            .get_conn_manager()
            .add_connector(TcpTunnelConnector::new(
                "tcp://10.1.2.3:52710".parse().unwrap(),
            ));
    } else if proto == "wg" {
        inst4
            .get_conn_manager()
            .add_connector(WgTunnelConnector::new(
                "wg://10.1.2.3:52711".parse().unwrap(),
                WgConfig::new_from_network_identity(
...
async fn avoid_tunnel_loop_back_to_virtual_network(#[values(true, false)] no_tun: bool) {
    ...
    let tcp_connector = TcpTunnelConnector::new("tcp://10.144.144.2:52710".parse().unwrap());
    insts[0]
        .get_peer_manager()
    ...
    let udp_connector = UdpTunnelConnector::new("udp://10.144.144.3:52710".parse().unwrap());
    insts[0]
        .get_peer_manager()
