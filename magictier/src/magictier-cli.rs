...
use magictier::service_manager::{Service, ServiceInstallOptions};
...
#[derive(Parser, Debug)]
#[command(name = "magictier-cli", author, version = MAGICTIER_VERSION, about, long_about = None)]
struct Cli {
    #[arg(
        short = 'p',
        long,
        default_value = "127.0.0.1:15888",
        help = "magictier-core rpc portal address"
    )]
...
enum SubCommand {
    ...
    VpnPortal,
    #[command(about = "inspect self magictier-core status")]
    Node(NodeArgs),
    #[command(about = "manage magictier-core as a system service")]
    Service(ServiceArgs),
...
enum ServiceSubCommand {
    #[command(about = "register magictier-core as a system service")]
    Install(InstallArgs),
    #[command(about = "unregister magictier-core system service")]
    Uninstall,
    #[command(about = "check magictier-core system service status")]
    Status,
    #[command(about = "start magictier-core system service")]
    Start,
    #[command(about = "stop magictier-core system service")]
    Stop,
}
...
struct InstallArgs {
    ...
    #[arg(long, help = "path to magictier-core binary")]
    ...
    #[arg(
        ...
        help = "args to pass to magictier-core"
    )]
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: magictier::proto::common::Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
...
"#,
            ...
            match service_args.sub_command {
                ServiceSubCommand::Install(install_args) => {
                    ...
                    let bin_path = std::fs::canonicalize(bin_path).map_err(|e| {
                        anyhow::anyhow!("failed to get magictier core application: {}", e)
                    })?;
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            magictier::print_completions(shell, &mut cmd, "magictier-cli");
        }
    }
