...
impl Default for MagicTierData {
    fn default() -> Self {
...
pub struct MagicTierLauncher {
    ...
    data: Arc<MagicTierData>,
}

impl MagicTierLauncher {
    pub fn new() -> Self {
        ...
        Self {
            ...
            data: Arc::new(MagicTierData::default()),
        }
    }

    async fn handle_magictier_event(event: GlobalCtxEvent, data: &MagicTierData) {
        ...
        tasks.spawn(async move {
            ...
            loop {
                match receiver.recv().await {
                    Ok(event) => {
                        Self::handle_magictier_event(event.clone(), &data_c).await;
                    }
        ...
        F: FnOnce() -> Result<TomlConfigLoader, anyhow::Error> + Send + Sync,
    {
        ...
        self.thread_handle = Some(std::thread::spawn(move || {
            ...
            let ret = rt.block_on(Self::magictier_routine(
                cfg,
                stop_notifier,
                api_service,
                data,
...
impl Default for MagicTierLauncher {
    fn default() -> Self {
        Self::new()
    }
}

impl Drop for MagicTierLauncher {
    fn drop(&mut self) {
        self.stop_flag
...
pub struct NetworkInstance {
    ...
    launcher: Option<MagicTierLauncher>,
...
impl NetworkInstance {
    ...
    pub fn is_magictier_running(&self) -> bool {
    ...
    pub async fn get_running_info(&self) -> anyhow::Result<NetworkInstanceRunningInfo> {
        ...
        Ok(NetworkInstanceRunningInfo {
            ...
            my_node_info: Some(MyNodeInfo {
                ...
                version: MAGICTIER_VERSION.to_string(),
                ips: my_info.ip_list,
    ...
    pub fn start(&mut self) -> Result<EventBusSubscriber, anyhow::Error> {
        if self.is_magictier_running() {
            return Ok(self.subscribe_event().unwrap());
        }

        let launcher = MagicTierLauncher::new();
