...
#[derive(Parser, Debug)]
#[command(name = "magictier-core", author, version = MAGICTIER_VERSION , about, long_about = None, disable_help_flag = true, disable_version_flag = true)]
struct Cli {
    #[clap(long = "zzhelp", help = "Print help information")]
    help: bool,

    #[clap(long = "zzversion", help = "Print version information")]
    version: bool,
    ...
    #[clap(short = 'z', long = "zzyczz", help = t!("core_clap.verbose").to_string())]
    verbose: bool,
}
...
impl NetworkOptions {
    ...
    fn merge_into(&self, cfg: &TomlConfigLoader) -> anyhow::Result<()> {
        ...
        if self.no_listener || !self.listeners.is_empty() {
            ...
            cfg.set_listeners(
                Cli::parse_listeners(false, vec!["52710".to_string()])?
                    .into_iter()
                    .map(|s| s.parse().unwrap())
                    .collect(),
            );
...
fn parse_cli() -> Cli {
    ...
    // 处理自定义的 help 和 version
    if cli.help {
        Cli::command().print_help().unwrap();
        std::process::exit(0);
    }
    if cli.version {
        println!("{}", MAGICTIER_VERSION);
        std::process::exit(0);
    }
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    // 设置 verbose 输出标志
    if cli.verbose {
        set_global_var!(VERBOSE_OUTPUT, true);
    }
    ...
    let _web_client = if let Some(config_server_url_s) = cli.config_server.as_ref() {
        ...
        .inspect(|_| {
            ...
            println!("Official config website: https://magictier.cn/web");
        })?;
    ...
    for config_file in config_files {
        ...
        if use_global_var!(VERBOSE_OUTPUT) {
            println!(
                "Starting magictier from config file {:?}({:?}) with config:",
...
pub async fn main() -> ExitCode {
    ...
    if let Some(shell) = cli.gen_autocomplete {
        ...
        crate::print_completions(shell, &mut cmd, "magictier-core");
