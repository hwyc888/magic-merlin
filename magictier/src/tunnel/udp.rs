...
async fn respond_stun_packet(
    ...
    // 使用混淆配置中的 STUN TID 前缀进行验证
    let expected_prefix = get_stun_tid_prefix_bytes();
    if tid.as_bytes()[0..4] != expected_prefix {
        anyhow::bail!("stun req tid not from magictier");
    }
...
mod tests {
    ...
    async fn ipv6_domain_pingpong() {
        ...
        let mut connector =
            UdpTunnelConnector::new("udp://test.magictier.top:31016".parse().unwrap());
