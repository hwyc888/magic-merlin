...
// 新增的规避技术模块 (2023-2024)
pub mod reality;           // REALITY 协议 - 最强规避
pub mod hysteria2;         // Hysteria 2 - 高速抗审查
pub mod shadowtls;         // ShadowTLS v3 - 完美 TLS 伪装
pub mod utls;              // uTLS 指纹伪装
pub mod traffic_shaping;   // 流量整形 - 对抗流量模式分析
pub mod domain_fronting;   // 域前置 - CDN 伪装
pub mod http2_disguise;    // HTTP/2 多路复用伪装
pub mod webrtc_tunnel;     // WebRTC 隧道 - 视频会议伪装

// 2024-2025 最新技术
pub mod tcp_fragment;      // TCP 分片 - 破坏 DPI 检测 (2025 增强: Geneva 策略)
pub mod ech;               // ECH - 加密 Client Hello，隐藏 SNI (2025 增强: HPKE)
pub mod xhttp;             // XHTTP - Xray 最新协议
pub mod brutal_cc;         // Brutal 拥塞控制 - 跑满带宽
pub mod multi_path;        // 多路径传输 - 分散流量特征
pub mod adversarial_ml;    // 对抗机器学习 - 欺骗 ML 分类器 (2025 增强: Markov 链)

// 2025 前沿技术
pub mod xtls_vision;       // XTLS Vision - 零拷贝 TLS，性能最强
pub mod grpc_disguise;     // gRPC 伪装 - API 调用伪装
pub mod tls_rotation;      // TLS 指纹轮换 - 动态改变指纹
pub mod smart_routing;     // 智能路由 - 强化学习路径选择
...
pub const PROTO_PORT_OFFSET: &[(&str, u16)] = &[
    ...
    ("wss", 2),
    ("faketcp", 3),
    ("reality", 4),      // REALITY 协议
    ("hysteria2", 5),    // Hysteria 2
    ("shadowtls", 6),    // ShadowTLS
];
